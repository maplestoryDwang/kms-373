...
public enum MonsterStatus implements Serializable {
   ...
   MS_Burned(101),
   MS_BalogDisable(102),
   MS_ExchangeAttack(103),
   MS_AddBuffStat(104),
   MS_LinkTeam(105),
   MS_SoulExplosion(106),
   ...
   private MonsterStatus(int flag) {
      this.i = 1 << 31 - flag % 32;
      this.first = 4 - (byte)((int)Math.floor((double)(flag / 32)));
      this.flag = flag;
      this.end = false;
      this.setStacked(this.name().startsWith("MS_Indie"));
   }
