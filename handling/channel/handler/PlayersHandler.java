...
public class PlayersHandler {
         ...
         if (chr.getLevel() < 15) {
            ...
            case OK:
               if (Math.abs(target.getFame() + famechange) <= 99999) {
                  target.updateSingleStat(MapleStat.FAME, (long)target.getFame());
               }
   ...
   public static void UseDoor(LittleEndianAccessor slea, MapleCharacter chr) {
      ...
      Iterator var4 = chr.getMap().getAllDoorsThreadsafe().iterator();
   ...
   public static void UseMechDoor(LittleEndianAccessor slea, MapleCharacter chr) {
      ...
      Iterator var5 = chr.getMap().getAllMechDoorsThreadsafe().iterator();
      ...
      if (c.getPlayer().getFollowId() > 0 && c.getPlayer().getFollowId() == slea.readInt()) {
         ...
         if (tt != null && tt.getTruePosition().distanceSq(c.getPlayer().getTruePosition()) < 10000.0D && tt.getFollowId() == 0 && tt.getId() != c.getPlayer().getId()) {
            ...
            if (accepted) {
               ...
               c.getPlayer().getMap().broadcastMessage(CField.followEffect(tt.getId(), c.getPlayer().getId(), (Point)null));
            } else {
         ...
         if (touched && reactor != null && reactor.isAlive() && reactor.getTouch() != 0) {
            ...
            if (reactor.getTouch() == 2) {
               ...
               if (reactor.getReactorType() == 100) {
                  int itemid = GameConstants.getCustomReactItem(reactor.getReactorId(), (Integer)reactor.getReactItem().getLeft());
                  if (c.getPlayer().haveItem(itemid, (Integer)reactor.getReactItem().getRight())) {
                     if (reactor.getArea().contains(c.getPlayer().getTruePosition())) {
                        MapleInventoryManipulator.removeById(c, GameConstants.getInventoryType(itemid), itemid, (Integer)reactor.getReactItem().getRight(), true, false);
   ...
   public static void DoRing(MapleClient c, String name, int itemid) {
      ...
      if (c.getPlayer().getMarriageId() > 0) {
         ...
         if (c.getPlayer().haveItem(itemid, 1) && itemid >= 2240000 && itemid <= 2240015) {
            if (chr.getMarriageId() <= 0 && chr.getMarriageItemId() <= 0) {
      ...
      if (errcode > 0) {
         c.getSession().writeAndFlush(CWvsContext.sendEngagement((byte)errcode, 0, (MapleCharacter)null, (MapleCharacter)null));
         c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
      } else {
   ...
   public static void RingAction(LittleEndianAccessor slea, MapleClient c) {
      ...
      if (mode == 0) {
         ...
         if (mode == 2) {
            ...
            i = slea.readInt();
            MapleCharacter chr = c.getChannelServer().getPlayerStorage().getCharacterByName(name);
            if (c.getPlayer().getMarriageId() > 0 || chr == null || chr.getId() != i || chr.getMarriageItemId() <= 0 || !chr.haveItem(chr.getMarriageItemId(), 1) || chr.getMarriageId() > 0 || !chr.isAlive() || chr.getEventInstance() != null || !c.getPlayer().isAlive() || c.getPlayer().getEventInstance() != null) {
               c.getSession().writeAndFlush(CWvsContext.sendEngagement((byte)33, 0, (MapleCharacter)null, (MapleCharacter)null));
               c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
               return;
            }
            ...
            if (accepted) {
               ...
               newItemId = itemid == 2240000 ? 1112803 : (itemid == 2240001 ? 1112806 : (itemid == 2240002 ? 1112807 : (itemid == 2240003 ? 1112809 : 1112300 + itemid - 2240004)));
               if (!MapleInventoryManipulator.checkSpace(c, newItemId, 1, "") || !MapleInventoryManipulator.checkSpace(chr.getClient(), newItemId, 1, "")) {
                  c.getSession().writeAndFlush(CWvsContext.sendEngagement((byte)25, 0, (MapleCharacter)null, (MapleCharacter)null));
                  c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
                  return;
               }
               ...
               try {
                  ...
                  FileoutputUtil.outputFileError("Log_Packet_Except.rtf", var13);
               }
            } else {
               chr.getClient().getSession().writeAndFlush(CWvsContext.sendEngagement((byte)34, 0, (MapleCharacter)null, (MapleCharacter)null));
            }
            ...
            if (mode == 3) {
               slot = slea.readInt();
               MapleInventoryType type = GameConstants.getInventoryType(slot);
               item = c.getPlayer().getInventory(type).findById(slot);
               if (item != null && type == MapleInventoryType.ETC && slot / 10000 == 421) {
               ...
               if (mode == 5) {
                  ...
                  itemid = slea.readInt();
                  i = slea.readInt();
                  data = MarriageManager.getInstance().getMarriage(c.getPlayer().getMarriageId());
                  if (data != null && data.getMarriageId() == itemid) {
                     Item invatation = c.getPlayer().getInventory(MapleInventoryType.ETC).getItem((short)i);
                     if (invatation != null && invatation.getItemId() == data.getTicketType().getInvitationItemId()) {
                        newItemId = World.Find.findChannel(receiver);
                        MapleCharacter chr = null;
                        if (newItemId >= 0) {
                           chr = ChannelServer.getInstance(newItemId).getPlayerStorage().getCharacterByName(receiver);
                        }

                        if (chr != null) {
                           if (data.getReservedPeopleList().contains(chr.getId())) {
                           ...
                           if (MapleInventoryManipulator.checkSpace(chr.getClient(), type.getInvitedItemId(), 1, "")) {
                              ...
                              c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
                              data.getReservedPeopleList().add(chr.getId());
                  ...
                  slot = slea.readInt();
                  itemid = slea.readInt();
                  item = c.getPlayer().getInventory(MapleInventoryType.ETC).getItem((short)slot);
                  if (item != null && item.getItemId() == itemid && item.getMarriageId() > 0) {
                     data = MarriageManager.getInstance().getMarriage(item.getMarriageId());
                     if (data != null) {
                  ...
                  c.getSession().writeAndFlush(CWvsContext.sendEngagement((byte)34, 0, (MapleCharacter)null, (MapleCharacter)null));
               } else if (mode == 9) {
                  ...
                  if (data != null && data.getStatus() == 1 && data.getWeddingStatus() >= 1 && data.getWeddingStatus() < 8) {
                     ...
                     MarriageTicketType type;
                     if (data.getGroomId() == c.getPlayer().getId()) {
                        type = data.getTicketType();
                        if (MapleInventoryManipulator.checkSpace(c, type.getInvitationItemId(), type.getInvitationQuantity(), "")) {
                           ...
                           c.getSession().writeAndFlush(CWvsContext.sendEngagement((byte)24, 0, (MapleCharacter)null, (MapleCharacter)null));
                           data.getGroomWishList().clear();
                        }
                     } else if (data.getBrideId() == c.getPlayer().getId()) {
                        type = data.getTicketType();
                     ...
                     if (data.getWeddingStatus() == 7) {
                        type = data.getTicketType();
                        c.getSession().writeAndFlush(CWvsContext.sendEngagement((byte)18, 0, (MapleCharacter)null, (MapleCharacter)null));
                        ...
                        if (channel >= 0) {
                           ...
                           if (chr != null) {
                              ...
                              chr.getClient().getSession().writeAndFlush(CWvsContext.sendEngagement((byte)18, 0, (MapleCharacter)null, (MapleCharacter)null));
                           }
                        }
   ...
   public static boolean inArea(MapleCharacter chr) {
      Iterator var1 = chr.getMap().getAreas().iterator();

      Rectangle rect;
      do {
         if (!var1.hasNext()) {
            var1 = chr.getMap().getAllMistsThreadsafe().iterator();
            ...
            do {
               ...
               mist = (MapleMist)var1.next();
            } while(mist.getOwnerId() != chr.getId() || mist.isPoisonMist() != 2 || !mist.getBox().contains(chr.getTruePosition()));

            return true;
         }

         rect = (Rectangle)var1.next();
      } while(!rect.contains(chr.getTruePosition()));
   ...
   public static final void TouchRune(LittleEndianAccessor slea, MapleCharacter chr) {
      ...
      Iterator var6 = chr.getCooldowns().iterator();

      MapleCoolDownValueHolder m;
      while(var6.hasNext()) {
         m = (MapleCoolDownValueHolder)var6.next();
      ...
      if (!GameConstants.isYeti(chr.getJob()) && !GameConstants.isPinkBean(chr.getJob())) {
         if (rune != null && rune.getRuneType() == type) {
            if (chr.getBuffedValue(80002282) && !chr.isGM()) {
               var6 = chr.getCooldowns().iterator();
         ...
         if (chr.getQuestStatus(501227) == 1) {
            if (chr.getKeyValue(501227, "RunAct") < 7L) {
               ...
               long var10003 = chr.getKeyValue(501227, "RunAct");
               chr.setKeyValue(501227, "RunAct", (var10003 + 1L).makeConcatWithConstants<invokedynamic>(var10003 + 1L));
         ...
         case 1:
            ...
            effect.applyTo(chr, false);
            SecondaryStatEffect a = SkillFactory.getSkill(80001431).getEffect(1);
            MapleMist mist = new MapleMist(a.calculateBoundingBox(chr.getPosition(), chr.isFacingLeft()), chr, a, 30000, (byte)(chr.isFacingLeft() ? 1 : 0));
         ...
         case 4:
            ...
            if (!spawn) {
               List<MapleMapObject> monstersInRange = chr.getMap().getMapObjectsInRange(chr.getPosition(), 2.0E7D, Arrays.asList(MapleMapObjectType.MONSTER));
            ...
            break;
         case 5:
            MapleMonster mob1 = MapleLifeFactory.getMonster(8220028);
         ...
         chr.getMap().setRune((MapleRune)null);
   ...
   public static void ColorCardHandler(LittleEndianAccessor slea, MapleClient c) {
      ...
      if (chr.getColorCardInstance() != null) {
         if ((Integer)chr.getColorCardInstance().getCardList().get(0) != type && type != 3) {
            ...
            switch((Integer)chr.getColorCardInstance().getCardList().get(0)) {
            case 0:
   ...
   public static void ContentsWaiting(LittleEndianAccessor slea, MapleClient c) {
      ...
      case 22:
         if (cancel) {
            c.send(SLFCGPacket.ContentsWaiting(c.getPlayer(), 0, 11, 5, 1, type));
            if (type == 21) {
      ...
      case 24:
         ...
         break;
      case 25:
         if (cancel && BloomingRace.getMatchinglist().contains(c.getPlayer())) {
            BloomingRace.getMatchinglist().remove(c.getPlayer());
         }
         break;
      case 26:
         if (cancel && BattleGroundGameHandler.getMatchinglist().contains(c.getPlayer())) {
   ...
   public static void MapleYutHandler(LittleEndianAccessor slea, MapleClient c) {
      ...
      if (c.getPlayer().getMultiYutInstance() != null) {
         ...
         case 0:
            ...
            break;
         case 1:
            int horseposition = slea.readInt();
            int yuttype = slea.readInt();
            int movecount = slea.readInt();
      ...
      if (c.getPlayer().getMtk() != null) {
         ...
         if (type == 0) {
            ...
            if (recipe.getDestination() == id) {
               ...
               c.getPlayer().getMtk().setRecipe((MapleTyoonKitchen.MapleTyoonKitchenRecipe)null);
               if (MapleTyoonKitchen.getAllMoney(c.getPlayer()) >= 500) {
                  ...
                  Iterator var17 = c.getPlayer().getMap().getAllChracater().iterator();

                  while(var17.hasNext()) {
                     MapleCharacter chr = (MapleCharacter)var17.next();
                     if (chr.getMtk() != null) {
            ...
            MapleTyoonKitchen.MapleTyoonKitchenRecipe recipe;
            if (number == 2) {
               recipe = (MapleTyoonKitchen.MapleTyoonKitchenRecipe)c.getPlayer().getMtk().getRecipes().get(id);
               ...
               recipe = (MapleTyoonKitchen.MapleTyoonKitchenRecipe)c.getPlayer().getMtk().getRecipes().get(id);
               Iterator var9 = recipe.getRecipeInfo().iterator();

               while(var9.hasNext()) {
                  Triple<Integer, Integer, Integer> list = (Triple)var9.next();
                  if ((Integer)list.getRight() == 0) {
                     if ((Integer)list.getLeft() != c.getPlayer().getMtk().getItemid()) {
                        String[] msg = new String[]{"오늘 장사는 접어야 할 것 같군.", "다시!", "손님들이 기다리다가 노인이 되겠군.", "빠르다고 다가 아니네!\r\n제대로 하게!", "지금 뭐 하는 건가!", "레시피를 제대로 보란 말일세!"};
                     ...
                     list.right = 1;
                     boolean suc = true;
                     Iterator var12 = recipe.getRecipeInfo().iterator();

                     while(var12.hasNext()) {
                        Triple<Integer, Integer, Integer> lists = (Triple)var12.next();
                        if ((Integer)lists.getRight() == 0) {
                           suc = false;
                           break;
                        }
                     }
