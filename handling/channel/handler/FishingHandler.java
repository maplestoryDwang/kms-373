...
public class FishingHandler {
   ...
   public static int[] items = new int[]{5064000, 5064100, 4035000, 4310283, 2049500, 2049700, 2438684, 2438679, 2438680, 5069100, 5068300, 5076000, 5062800, 5133000, 2435719};
   ...
   public static void fishing(LittleEndianAccessor slea, MapleClient c) {
      ...
      case 0:
         for(int i = 2; i <= 5; ++i) {
            if (i != 3 && c.getPlayer().getInventory((byte)i).getNextFreeSlot() == -1) {
      ...
      case 1:
         ...
         if (result == 1) {
            ...
            if (c.getPlayer().getV("fishing") != null) {
               ...
               c.getPlayer().addKV("fishing", k.makeConcatWithConstants<invokedynamic>(k));
            } else {
         ...
         for(int j = 2; j <= 5; ++j) {
            if (j != 3 && c.getPlayer().getInventory((byte)j).getNextFreeSlot() == -1) {
               c.getPlayer().dropMessage(1, "인벤토리의 공간이 부족합니다.");
               return;
            }
         }
