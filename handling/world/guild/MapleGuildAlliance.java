...
public class MapleGuildAlliance implements Serializable {
   ...
   public final boolean deleteAlliance() {
      ...
      try {
         ...
         System.err.println("SQL THROW" + var15);
         var5 = false;
      } finally {
         try {
            if (con != null) {
   ...
   public final void broadcast(byte[] packet) {
      this.broadcast(packet, -1, MapleGuildAlliance.GAOp.NONE, false);
   }

   public final void broadcast(byte[] packet, int exception) {
      this.broadcast(packet, exception, MapleGuildAlliance.GAOp.NONE, false);
   }

   public final void broadcast(byte[] packet, int exceptionId, MapleGuildAlliance.GAOp op, boolean expelled) {
      if (op == MapleGuildAlliance.GAOp.DISBAND) {
         World.Alliance.setOldAlliance(exceptionId, expelled, this.allianceid);
      } else if (op == MapleGuildAlliance.GAOp.NEWGUILD) {
   ...
   public final boolean disband() {
      ...
      if (ret) {
         this.broadcast((byte[])null, -1, MapleGuildAlliance.GAOp.DISBAND, false);
      }
   ...
   public void setRank(String[] ranks) {
      this.ranks = ranks;
      this.broadcast(CWvsContext.AlliancePacket.allianceRankChange(this.allianceid, ranks));
      this.saveToDb();
   }
   ...
   public boolean addGuild(int guildid) {
      if (this.getNoGuilds() >= this.getCapacity()) {
         ...
         this.saveToDb();
         this.broadcast((byte[])null, guildid, MapleGuildAlliance.GAOp.NEWGUILD, false);
         return true;
      }
   }
   ...
   public boolean removeGuild(int guildid, boolean expelled, boolean isNull) {
      for(int i = 0; i < this.getNoGuilds(); ++i) {
         if (this.guilds[i] == guildid) {
            if (!isNull) {
               this.broadcast((byte[])null, guildid, MapleGuildAlliance.GAOp.DISBAND, expelled);
            }
            ...
            this.broadcast(CWvsContext.AlliancePacket.getAllianceUpdate(this));
            this.broadcast(CWvsContext.AlliancePacket.getGuildAlliance(this));
            this.saveToDb();
            return true;
         }
      }
   ...
   public boolean setLeaderId(int c, boolean sameGuild) {
      if (this.leaderid == c) {
         ...
         if (g == -1) {
            ...
            this.guilds[0] = i;
            if (leaderName != null) {
               this.broadcast(CWvsContext.serverNotice(5, "", leaderName + " has become the leader of the alliance."));
            }

            this.broadcast(CWvsContext.AlliancePacket.changeAllianceLeader(this.allianceid, this.leaderid, c));
            this.broadcast(CWvsContext.AlliancePacket.updateAllianceLeader(this.allianceid, this.leaderid, c));
            this.broadcast(CWvsContext.AlliancePacket.getAllianceUpdate(this));
            this.broadcast(CWvsContext.AlliancePacket.getGuildAlliance(this));
            this.leaderid = c;
            this.saveToDb();
