...
public class CashCalcInsertData {
   ...
   public static void main(String[] args) {
      ...
      for(int i = 0; i < strs.length; ++i) {
         ...
         data = strs[i].split("\\|");
         Iterator var5 = MapleItemInformationProvider.getInstance().getAllItems().iterator();

         while(var5.hasNext()) {
            Pair<Integer, String> itemPair = (Pair)var5.next();
            if (((String)itemPair.getRight()).toLowerCase().equals(data[0])) {
               retItems.add(new Triple(itemPair.getLeft() + "", (String)itemPair.getRight(), data[1]));
            }
         }
         ...
         if (retItems != null && retItems.size() > 0) {
            if (retItems.size() == 1) {
               itemcodes.add((Triple)retItems.get(0));
            } else {
               System.out.println("중복리스트가 존재");

               int geti;
               for(geti = 0; geti < retItems.size(); ++geti) {
                  System.out.println(geti + 1 + ". " + (String)((Triple)retItems.get(geti)).getLeft() + " - " + (String)((Triple)retItems.get(geti)).getMid());
               }

               geti = sc.nextInt();
               if (geti != -1) {
                  itemcodes.add((Triple)retItems.get(geti - 1));
               }
            }
         }
      }
