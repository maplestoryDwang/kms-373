...
public class OXQuizGame {
   ...
   public void InitGame() {
      ...
      Iterator var1 = this.chars.iterator();

      while(var1.hasNext()) {
         MapleCharacter chr = (MapleCharacter)var1.next();
         chr.getClient().getSession().writeAndFlush(SLFCGPacket.OXQuizExplain("문제를 많이 맞힐수록 더 많은 보상이 기다리고 있지. 너의 상식은 어디까지?"));
      }

      Timer.EventTimer.getInstance().schedule(new Runnable() {
         public void run() {
            Iterator var1 = OXQuizGame.this.chars.iterator();

            while(var1.hasNext()) {
               MapleCharacter chr = (MapleCharacter)var1.next();
               chr.getClient().getSession().writeAndFlush(SLFCGPacket.OXQuizExplain("확실히 선택해야 해. 가운데 애매하게 걸치면 광속으로 탈락할 수 있다구!"));
            }
      ...
      Timer.EventTimer.getInstance().schedule(new Runnable() {
         public void run() {
            ...
            while(var1.hasNext()) {
               MapleCharacter chr = (MapleCharacter)var1.next();
               chr.getClient().getSession().writeAndFlush(SLFCGPacket.OXQuizExplain("모두들 밑으로 내려가서 문제를 잘 들어봐. 맞다고 생각하면 O! 틀린 것 같다면 X!"));
            }
      ...
      Timer.EventTimer.getInstance().schedule(new Runnable() {
         public void run() {
            ...
            while(var1.hasNext()) {
               MapleCharacter chr = (MapleCharacter)var1.next();
               chr.getClient().getSession().writeAndFlush(SLFCGPacket.OXQuizExplain("왼쪽? 오른쪽? 숫자가 모두 카운트되기 전에 확실히 이동하라구!"));
            }
      ...
      Timer.EventTimer.getInstance().schedule(new Runnable() {
         public void run() {
            ...
            while(var1.hasNext()) {
               MapleCharacter chr = (MapleCharacter)var1.next();
               chr.getClient().getSession().writeAndFlush(SLFCGPacket.OXQuizExplain("그럼 지금부터 시작한다, 스릴 넘치는 OX 퀴즈!"));
   ...
   public void StartQuiz() {
      this.QuizTimer = Timer.EventTimer.getInstance().register(new Runnable() {
         ...
         public void run() {
            ...
            if (this.count == 0) {
               ++this.index;
               this.tempq = (OXQuiz)OXQuizGame.this.quizes.get(this.index);
               var1 = OXQuizGame.this.chars.iterator();
               ...
               while(true) {
                  while(true) {
                     while(var1.hasNext()) {
                        ...
                        if (chr.getPosition().getY() >= 89.0D && chr.getPosition().getY() <= 576.0D) {
                           ...
                           if (chr.getPosition().getX() <= -752.0D && chr.getPosition().getX() >= -1450.0D) {
                              if (this.tempq.isX()) {
                                 ...
                                 amount = (this.index + 1) * 2;
                                 chr.AddStarDustCoin(2, 100);
                              }
                           } else if (chr.getPosition().getX() <= -500.0D && chr.getPosition().getX() >= -753.0D) {
                              OXQuizGame.this.deadchars.add(chr);
                              chr.getClient().getSession().writeAndFlush(SLFCGPacket.OXQuizTelePort((byte)10));
                           } else if (this.tempq.isX()) {
                              amount = (this.index + 1) * 2;
                              chr.AddStarDustCoin(2, 100);
                           } else {
                              chr.getClient().getSession().writeAndFlush(SLFCGPacket.OXQuizTelePort((byte)10));
                        ...
                        } else if (!OXQuizGame.this.deadchars.contains(chr)) {
                           OXQuizGame.this.deadchars.add(chr);
                        }
                     }
