...
public class BattleGroundGameHandler {
   ...
   private static boolean NotDamage = false;
   ...
   public static void AddChr(MapleCharacter chr) {
      ...
      chr.getClient().send(SLFCGPacket.ContentsWaiting(chr, 993026900, 11, 2, 1, 26));
   }
   ...
   public static void AddChangeChannelChr(int charid) {
      ...
      while(var1.hasNext()) {
         ...
         if (charid == chrs.getId()) {
            ...
            chrs.getClient().send(SLFCGPacket.ContentsWaiting(chrs, 993026900, 11, 2, 1, 26));
            break;
         }
      }
   ...
   public static void Ready() {
      ...
      while(totalPeople > 0) {
         Iterator var7 = Matchinglist.iterator();
         ...
         for(Iterator var16 = chrs.iterator(); var16.hasNext(); ++portal) {
            MapleCharacter chr = (MapleCharacter)var16.next();
            ...
            Iterator var17 = chr.getEffects().iterator();

            while(var17.hasNext()) {
               Pair<SecondaryStat, SecondaryStatValueHolder> data = (Pair)var17.next();
               SecondaryStatValueHolder mbsvh = (SecondaryStatValueHolder)data.right;
               if (SkillFactory.getSkill(mbsvh.effect.getSourceId()) != null && mbsvh.effect.getSourceId() != 80002282 && mbsvh.effect.getSourceId() != 2321055) {
                  chr.cancelEffect(mbsvh.effect, Arrays.asList((SecondaryStat)data.left));
               }
   ...
   public static void StartGame(MapleMap map) {
      if (map != null && map.getAllChracater().size() > 0) {
         ...
         while(true) {
            ...
            } while(chr.getBattleGroundChr() != null);

            int type;
            for(type = Randomizer.rand(1, 11); type == 8; type = Randomizer.rand(1, 11)) {
            }
