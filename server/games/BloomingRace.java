...
public class BloomingRace {
   ...
   public static void RaceIinviTation() {
      ...
      while(var0.hasNext()) {
         ChannelServer cserv = (ChannelServer)var0.next();
         Iterator var2 = cserv.getPlayerStorage().getAllCharacters().values().iterator();

         while(var2.hasNext()) {
            MapleCharacter player = (MapleCharacter)var2.next();
   ...
   public static void RaceAddChr(MapleCharacter chr) {
      ...
      chr.getClient().send(SLFCGPacket.ContentsWaiting(chr, 993192500, 11, 2, 1, 25));
   }

   public static void RaceAddChangeChannelChr(int charid) {
      Iterator var1 = ChannelServer.getInstance(1).getPlayerStorage().getAllCharacters().values().iterator();

      while(var1.hasNext()) {
         MapleCharacter chrs = (MapleCharacter)var1.next();
         if (charid == chrs.getId()) {
            Matchinglist.add(chrs);
            chrs.getClient().send(SLFCGPacket.ContentsWaiting(chrs, 993192500, 11, 2, 1, 25));
            break;
         }
      }
   ...
   public static void RaceWarp() {
      ...
      Iterator var0 = Matchinglist.iterator();

      while(var0.hasNext()) {
         MapleCharacter chr = (MapleCharacter)var0.next();
         Iterator var2 = chr.getEffects().iterator();

         while(var2.hasNext()) {
            Pair<SecondaryStat, SecondaryStatValueHolder> data = (Pair)var2.next();
   ...
   public static void ReadyRace() {
      ...
      Iterator var3 = map.getAllChracater().iterator();
      ...
      playmap.broadcastMessage(SLFCGPacket.BloomingRaceHandler(2, 30000));
      Timer.EventTimer.getInstance().schedule(() -> {
      ...
      Timer.EventTimer.getInstance().schedule(() -> {
         playmap.broadcastMessage(CField.enforceMSG("블루밍 레이스에 온걸 환영해! 신나게 달릴 준비는 됐어?", 287, 3500));
      }, 6000L);
      Timer.EventTimer.getInstance().schedule(() -> {
         playmap.broadcastMessage(CField.enforceMSG("친구들이 심심하지 않게 여러가지 코스로 준비했으니 재미있게 즐겨줘.", 287, 3500));
      }, 10000L);
      Timer.EventTimer.getInstance().schedule(() -> {
         playmap.broadcastMessage(CField.enforceMSG("생각만큼 가는 길이 쉽지는 않을 거니까, 긴장하는 게 좋을 거야.", 287, 3500));
      }, 14000L);
      Timer.EventTimer.getInstance().schedule(() -> {
         playmap.broadcastMessage(CField.enforceMSG("히힛, 하지만 친구들이라면 이 정도는 충분히 가능할 거라고 믿어.", 287, 3500));
      }, 18000L);
      Timer.EventTimer.getInstance().schedule(() -> {
         playmap.broadcastMessage(CField.enforceMSG("다른 친구들보다 먼저 도착하면 더 좋은 선물을 줄게. 준비됐어?", 287, 3500));
      }, 22000L);
   }
   ...
   public static void StartRace() {
      ...
      playmap.broadcastMessage(SLFCGPacket.BloomingRaceHandler(4, 300000));
   }
