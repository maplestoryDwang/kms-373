...
public class MapleFlyingSword extends MapleMapObject {
   ...
   public void updateFlyingSwordNode(MapleMap map) {
      ++this.count;
      this.getNodes().clear();
      List<FlyingSwordNode> node = this.setNodes();
      this.setNodes(node);
   ...
   public void tryAttack(final MapleMap map, Point pos) {
      this.setStop(true);
      MapleCharacter target = map.getCharacter(this.getTarget().getId());
      if (target != null) {
         ...
         this.setPosition(new Point(pos.x, 16));
         if (mist != null) {
   ...
   public void updateTarget(MapleMap map) {
      if (map.getCharactersThreadsafe().size() > 0) {
         MapleCharacter target = (MapleCharacter)map.getCharactersThreadsafe().get(Randomizer.nextInt(map.getCharactersThreadsafe().size()));
         if (target != null) {
