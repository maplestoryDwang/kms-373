...
public class MapleMiniGame extends AbstractPlayerStore {
   ...
   public int getCardId(int slot) {
      return (Integer)this.matchcards.get(slot - 1);
   }
   ...
   public void exit(MapleCharacter player) {
      if (player != null) {
         player.setPlayerShop((IMaplePlayerShop)null);
   ...
   public void checkExitAfterGame() {
      for(int i = 0; i < 2; ++i) {
         if (this.exitAfter[i]) {
            this.exitAfter[i] = false;
            (i == 0 ? this.getMCOwner() : (MapleCharacter)this.chrs[i - 1].get()).getClient().getSession().writeAndFlush(PlayerShopPacket.shopErrorMessage(0, i));
            if (i == 0 && this.getShopType() != 1) {
               this.closeShop(false, this.isAvailable());
            } else {
               (i == 0 ? this.getMCOwner() : (MapleCharacter)this.chrs[i - 1].get()).setPlayerShop((IMaplePlayerShop)null);
               this.removeVisitor((MapleCharacter)this.chrs[i - 1].get());
            }
         }
      }
