...
public class MapleQuest implements Serializable {
   ...
   private static MapleQuest loadQuest(ResultSet rs, PreparedStatement psr, PreparedStatement psa, PreparedStatement pss, PreparedStatement psq, PreparedStatement psi, PreparedStatement psp) throws SQLException {
      ...
      while(rse.next()) {
         ...
         if (type.equals(MapleQuestRequirementType.interval)) {
            ...
            Iterator var11 = req.getDataStore().iterator();

            while(var11.hasNext()) {
               Pair<Integer, Integer> mob = (Pair)var11.next();
      ...
      while(true) {
         MapleQuestActionType ty;
         label89:
         do {
            while(rse.next()) {
               ty = MapleQuestActionType.getByWZName(rse.getString("name"));
               if (rse.getInt("type") == 0) {
                  continue label89;
               }
            ...
            for(rse = psp.executeQuery(); rse.next(); ((List)ret.partyQuestInfo.get(rse.getString("rank"))).add(new Pair(rse.getString("mode"), new Pair(rse.getString("property"), rse.getInt("value"))))) {
            ...
            rse.close();
            return ret;
         } while(ty == MapleQuestActionType.item && ret.id == 7103);
   ...
   public boolean canStart(MapleCharacter c, Integer npcid) {
      if (c.getQuest(this).getStatus() == 0 || c.getQuest(this).getStatus() == 2 && this.repeatable) {
         if (this.blocked && !c.isGM()) {
            ...
            Iterator var3 = this.startReqs.iterator();

            MapleQuestRequirement r;
            do {
               if (!var3.hasNext()) {
                  return true;
               }

               r = (MapleQuestRequirement)var3.next();
               if (r.getType() == MapleQuestRequirementType.dayByDay && npcid != null) {
                  this.forceComplete(c, npcid);
                  return false;
               }
            } while(r.check(c, npcid));
   ...
   public boolean canComplete(MapleCharacter chr, Integer npcid) {
      return this.canComplete(chr, npcid, false);
   }
   ...
   public boolean canComplete(MapleCharacter chr, Integer npcid, boolean acc) {
      ...
      if (this.blocked && !chr.isGM()) {
         return false;
      } else if (this.autoComplete && npcid != null && this.viewMedalItem <= 0) {
         this.forceComplete(chr, npcid, acc);
         return false;
      } else {
         Iterator var4 = this.completeReqs.iterator();
         ...
         do {
            ...
            r = (MapleQuestRequirement)var4.next();
         } while(r.check(chr, npcid));
   ...
   public final void RestoreLostItem(MapleCharacter c, int itemid) {
      if (!this.blocked || c.isGM()) {
         Iterator var3 = this.startActs.iterator();

         while(var3.hasNext()) {
            MapleQuestAction a = (MapleQuestAction)var3.next();
   ...
   public void start(MapleCharacter c, int npc) {
      this.start(c, npc, false);
   }

   public void start(MapleCharacter c, int npc, boolean acc) {
      if (this.autoStart || this.checkNPCOnMap(c, npc)) {
         Iterator var4 = this.startActs.iterator();

         MapleQuestAction a;
         while(var4.hasNext()) {
            a = (MapleQuestAction)var4.next();
            if (!a.checkEnd(c, (Integer)null)) {
               return;
            }
         }

         var4 = this.startActs.iterator();

         while(var4.hasNext()) {
            a = (MapleQuestAction)var4.next();
            a.runStart(c, (Integer)null);
         }

         if (!this.customend) {
            this.forceStart(c, npc, (String)null, acc);
   ...
   public void complete(MapleCharacter c, int npc) {
      this.complete(c, npc, (Integer)null, false);
   }

   public void complete(MapleCharacter c, int npc, boolean acc) {
      this.complete(c, npc, (Integer)null, acc);
   }

   public void complete(MapleCharacter c, int npc, Integer selection, boolean acc) {
      if (this.getId() == 100855) {
         this.forceComplete(c, npc, true);
      } else {
         if (c.getMap() != null && (this.autoPreComplete || this.checkNPCOnMap(c, npc)) && this.canComplete(c, npc)) {
            Iterator var5 = this.completeActs.iterator();

            MapleQuestAction a;
            while(var5.hasNext()) {
               a = (MapleQuestAction)var5.next();
               if (!a.checkEnd(c, selection)) {
                  return;
               }
            }

            this.forceComplete(c, npc, acc);
            var5 = this.completeActs.iterator();
   ...
   public void forceStart(MapleCharacter c, int npc, String customData) {
      this.forceStart(c, npc, customData, false);
   }
   ...
   public void forceComplete(MapleCharacter c, int npc) {
      this.forceComplete(c, npc, true, false);
   }

   public void forceComplete(MapleCharacter c, int npc, boolean update) {
      this.forceComplete(c, npc, update, false);
   }

   public void forceComplete(MapleCharacter c, int npc, boolean update, boolean acc) {
      if (this.getId() == 100855 || (this.getId() < 100829 || this.getId() > 100863) && (this.getId() < 501460 || this.getId() > 501592)) {
         ...
         if (c.getClient().getQuestStatus(100825) == 2) {
            if (this.id >= 34130 && this.id <= 34150) {
               if (c.getKeyValue(34127, "count") >= 5L && c.getKeyValue(100837, "state") != 2L) {
                  c.setKeyValue(100837, "state", "1");
               }
            } else if (this.id >= 39017 && this.id <= 39033) {
               if (c.getKeyValue(39016, "count") >= 3L && c.getKeyValue(100839, "state") != 2L) {
                  c.setKeyValue(100839, "state", "1");
               }
            } else if (this.id >= 34381 && this.id <= 34394) {
               if (c.getKeyValue(34380, "count") >= 3L && c.getKeyValue(100841, "state") != 2L) {
                  c.setKeyValue(100841, "state", "1");
               }
            } else if (this.id >= 39038 && this.id <= 34394) {
               if (c.getKeyValue(39037, "count") >= 3L && c.getKeyValue(100843, "state") != 2L) {
                  c.setKeyValue(100843, "state", "1");
               }
            } else if (this.id >= 34276 && this.id <= 34295) {
               if (c.getKeyValue(34298, "count") >= 3L && c.getKeyValue(100845, "state") != 2L) {
            ...
            } else if (this.id >= 39820 && this.id <= 39825) {
               if (c.getKeyValue(34776, "count") >= 3L && c.getKeyValue(100847, "state") != 2L) {
            ...
            } else if (this.id >= 38151 && this.id <= 38156 && c.getKeyValue(100849, "state") != 2L) {
         ...
         case 16408:
            ...
            long var10003 = c.getKeyValue(500862, "openBox") + 1L;
            c.setKeyValue(500862, "str", "일일 미션 " + var10003 + "개 완료! " + (c.getKeyValue(500862, "openBox") + 1L) + "번째 상자를 클릭하세요!");
            if (c.getKeyValue(500862, "M5") > 0L) {
               c.setKeyValue(500862, "M6", "1");
            } else if (c.getKeyValue(500862, "M4") > 0L) {
               c.setKeyValue(500862, "M5", "1");
            } else if (c.getKeyValue(500862, "M3") > 0L) {
               c.setKeyValue(500862, "M4", "1");
            } else if (c.getKeyValue(500862, "M2") > 0L) {
               c.setKeyValue(500862, "M3", "1");
            } else if (c.getKeyValue(500862, "M1") > 0L) {
         ...
         if (this.getId() >= 100829 && this.getId() <= 100863) {
            if (c.getKeyValue(this.getId(), "state") != 2L) {
               c.setKeyValue(this.getId(), "state", "1");
            }
         } else if (c.getClient().getCustomKeyValue(this.getId(), "state") != 2L) {
            c.getClient().setCustomKeyValue(this.getId(), "state", "1");
         }
