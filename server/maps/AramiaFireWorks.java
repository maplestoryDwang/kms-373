...
public class AramiaFireWorks {
   ...
   private static final int[] arrayMob = new int[]{9500168, 9500169, 9500170, 9500171, 9500173, 9500174, 9500175, 9500176, 9500170, 9500171, 9500172, 9500173, 9500174, 9500175, 9400569};
   private static final int[] arrayX = new int[]{2100, 2605, 1800, 2600, 3120, 2700, 2320, 2062, 2800, 3100, 2300, 2840, 2700, 2320, 1950};
   private static final int[] arrayY = new int[]{574, 364, 574, 316, 574, 574, 403, 364, 574, 574, 403, 574, 574, 403, 574};
   ...
   private final void broadcastServer(MapleCharacter c, int itemid) {
      int var10002 = c.getClient().getChannel();
      World.Broadcast.broadcastMessage(CWvsContext.serverNotice(6, "", "<Channel " + var10002 + "> " + c.getMap().getMapName() + " : The amount of {" + MapleItemInformationProvider.getInstance().getName(itemid) + "} has reached the limit!"));
   }
   ...
   private final void broadcastEvent(final MapleCharacter c) {
      this.broadcastServer(c, 4001128);
