...
public class MapleMapFactory {
   ...
   public final MapleMap getMap(int mapid) {
      ...
      try {
         asdf = this.getMap(mapid, true, true, true);
   ...
   public final MapleMap getMap(int mapid, boolean respawns, boolean npcs) {
      ...
      try {
         asdf = this.getMap(mapid, respawns, npcs, true);
   ...
   public final MapleMap getMap(int mapid, boolean respawns, boolean npcs, boolean reactors) {
      ...
      if (map == null) {
         ...
         try {
            ...
            Iterator var14 = mapData.getChildByPath("foothold").iterator();

            while(var14.hasNext()) {
               MapleData footRoot = (MapleData)var14.next();
               Iterator var16 = footRoot.iterator();

               while(var16.hasNext()) {
                  MapleData footCat = (MapleData)var16.next();

                  MapleFoothold fh;
                  for(Iterator var18 = footCat.iterator(); var18.hasNext(); allFootholds.add(fh)) {
                     MapleData footHold = (MapleData)var18.next();
            ...
            Iterator var88 = allFootholds.iterator();

            while(var88.hasNext()) {
               MapleFoothold foothold = (MapleFoothold)var88.next();
            ...
            int lowestLevel = 200;
            int highestLevel = 0;
            Iterator var95 = mapData.getChildByPath("life").iterator();
                     ...
                     do {
                        if (!var95.hasNext()) {
                           ...
                           if (reactors && mapData.getChildByPath("reactor") != null) {
                              Iterator var98 = mapData.getChildByPath("reactor").iterator();

                              while(var98.hasNext()) {
                                 MapleData reactor = (MapleData)var98.next();
                           ...
                           if (reactors && map.monsterSpawn.size() > 0 && map.isSpawnPoint() && !GameConstants.보스맵(map.getId()) && !GameConstants.사냥컨텐츠맵(map.getId()) && !GameConstants.튜토리얼(map.getId()) && !GameConstants.로미오줄리엣(map.getId()) && !GameConstants.피라미드(map.getId()) && !GameConstants.isContentsMap(map.getId())) {
                              ...
                              allowedSpawn.add(100012);
                              allowedSpawn.add(100013);
                              allowedSpawn.add(200011);
                              allowedSpawn.add(200012);
                              allowedSpawn.add(200013);
                              ...
                              for(int i = 0; i < 1 && !herbRocks.isEmpty(); ++i) {
                                 ...
                                 life = Randomizer.rand(0, map.monsterSpawn.size() - 1);
                                 MapleRune rune = map.getRune();
                                 if (rune != null) {
                                    while(life == rune.getSpawnPointNum()) {
                                       life = Randomizer.rand(0, map.monsterSpawn.size() - 1);
                                    }
                                 }

                                 Point poss = ((Spawns)map.monsterSpawn.get(life)).getPosition();
                                 ...
                                 myReactor.setSpawnPointNum(life);
                           ...
                           try {
                              ...
                              sql = "SELECT * FROM `spawn` WHERE mapid = " + mapid + " AND type = 'r'";
                              if (con != null) {
                                 ...
                                 if (ps != null) {
                                    ...
                                    while(rs.next()) {
                                       life = rs.getInt("lifeid");
                                       MapleReactor reactor = new MapleReactor(MapleReactorFactory.getReactor(life), life);
                                       if (reactor == null) {
                                          PrintStream var10000 = System.out;
                                          String var10001 = reactor.getName();
                                          var10000.println("리엑터 null 발생!! " + var10001 + ", " + life);
                                       } else {
                           ...
                           try {
                              con = DatabaseConnection.getConnection();
                              sql = "SELECT * FROM `spawn` WHERE mapid = " + mapid + " AND type = 'n'";
                              if (con != null) {
                                 ps = con.prepareStatement(sql);
                                 if (ps != null) {
                                    rs = ps.executeQuery();
                           ...
                           try {
                              map.setMapName(MapleDataTool.getString("mapName", this.nameData.getChildByPath(this.getMapStringName(omapid)), ""));
                              map.setStreetName(MapleDataTool.getString("streetName", this.nameData.getChildByPath(this.getMapStringName(omapid)), ""));
                           ...
                           if (mapData.getChildByPath("monsterDefense/wave") != null) {
                              ...
                              int w = false;
                              Iterator var116 = wavedata.getChildren().iterator();
                              ...
                              while(true) {
                                 ...
                                 MapleData wave = (MapleData)var116.next();
                                 int i = 0;
                                 new ArrayList();

                                 for(Iterator var31 = wave.getChildren().iterator(); var31.hasNext(); ++i) {
                                    MapleData mid = (MapleData)var31.next();
                                    if (Integer.parseInt(wave.getName()) != 20 && (i == 0 || i % 4 == 0) || Integer.parseInt(wave.getName()) == 20 && (i == 0 || i % 2 == 0)) {
                                       if (monster.get(Integer.parseInt(wave.getName())) == null) {
                                          monster.put(Integer.parseInt(wave.getName()), new ArrayList());
                                       }

                                       ((List)monster.get(Integer.parseInt(wave.getName()))).add(MapleDataTool.getInt(mid.getChildByPath("mobID"), 0));
                                    }
                                 }
                              }
                           }
                        ...
                        life = (MapleData)var95.next();
                        type = MapleDataTool.getString(life.getChildByPath("type"));
                        id = MapleDataTool.getString("limitedname", life, "");
                     } while(!npcs && type.equals("n"));
                  } while(id.equals("Stage0"));
   ...
   public MapleMap CreateInstanceMap(int mapid, boolean respawns, boolean npcs, boolean reactors, int instanceid) {
      ...
      try {
         if (this.isInstanceMapLoaded(instanceid)) {
            var6 = this.getInstanceMap(instanceid);
            return var6;
         }
      } finally {
         this.lock.unlock();
      ...
      if (mapData == null) {
         ...
         Point lBound = new Point();
         Point uBound = new Point();
         Iterator var13 = mapData.getChildByPath("foothold").iterator();

         while(var13.hasNext()) {
            MapleData footRoot = (MapleData)var13.next();
            Iterator var15 = footRoot.iterator();

            while(var15.hasNext()) {
               MapleData footCat = (MapleData)var15.next();

               MapleFoothold fh;
               for(Iterator var17 = footCat.iterator(); var17.hasNext(); allFootholds.add(fh)) {
                  MapleData footHold = (MapleData)var17.next();
                  fh = new MapleFoothold(new Point(MapleDataTool.getInt(footHold.getChildByPath("x1")), MapleDataTool.getInt(footHold.getChildByPath("y1"))), new Point(MapleDataTool.getInt(footHold.getChildByPath("x2")), MapleDataTool.getInt(footHold.getChildByPath("y2"))), Integer.parseInt(footHold.getName()));
         ...
         MapleFootholdTree fTree = new MapleFootholdTree(lBound, uBound);
         Iterator var38 = allFootholds.iterator();

         while(var38.hasNext()) {
            MapleFoothold fh = (MapleFoothold)var38.next();
            fTree.insert(fh);
         }
         ...
         Iterator var42 = mapData.getChildByPath("life").iterator();
               ...
               do {
                  do {
                     if (!var42.hasNext()) {
                        ...
                        map.setNodes(this.loadNodes(mapid, mapData));
                        if (reactors && mapData.getChildByPath("reactor") != null) {
                           var42 = mapData.getChildByPath("reactor").iterator();

                           while(var42.hasNext()) {
                              reactor = (MapleData)var42.next();
                              id = MapleDataTool.getString(reactor.getChildByPath("id"));
                              if (id != null) {
                                 map.spawnReactor(this.loadReactor(reactor, id, (byte)MapleDataTool.getInt(reactor.getChildByPath("f"), 0)));
                              }
                     ...
                     reactor = (MapleData)var42.next();
                     id = MapleDataTool.getString(reactor.getChildByPath("type"));
                     limited = MapleDataTool.getString("limitedname", reactor, "");
                  } while(!npcs && id.equals("n"));
               } while(!limited.equals(""));

               AbstractLoadedMapleLife myLife = this.loadLife(reactor, MapleDataTool.getString(reactor.getChildByPath("id")), id);
               if (myLife instanceof MapleMonster && !GameConstants.isNoSpawn(mapid)) {
                  MapleMonster mob = (MapleMonster)myLife;
                  map.addMonsterSpawn(mob, MapleDataTool.getInt("mobTime", reactor, 0), (byte)MapleDataTool.getInt("team", reactor, -1), mob.getId() == bossid ? msg : null);
                  if (map.getBurning() == 0) {
   ...
   private void loadPortals(MapleMap map, MapleData port) {
      if (port != null) {
         ...
         MaplePortal myPortal;
         for(Iterator var4 = port.getChildren().iterator(); var4.hasNext(); map.addPortal(myPortal)) {
            MapleData portal = (MapleData)var4.next();
            myPortal = new MaplePortal(MapleDataTool.getInt(portal.getChildByPath("pt")));
            ...
            String script = MapleDataTool.getString("script", portal, (String)null);
   ...
   private MapleNodes loadNodes(int mapid, MapleData mapData) {
      ...
      if (mapData.getChildByPath("nodeInfo") != null) {
         var4 = mapData.getChildByPath("nodeInfo").iterator();

         while(var4.hasNext()) {
            node = (MapleData)var4.next();
            ...
            try {
               if (node.getName().equals("start")) {
                  ...
                  if (node.getChildByPath("edge") != null) {
                     Iterator var7 = node.getChildByPath("edge").iterator();

                     while(var7.hasNext()) {
                        mc2 = (MapleData)var7.next();
                        edges.add(MapleDataTool.getInt(mc2, -1));
                     }
                  }
      ...
      for(int i = 0; i <= 10; ++i) {
         if (mapData.getChildByPath(String.valueOf(i)) != null && mapData.getChildByPath(i + "/obj") != null) {
            var14 = mapData.getChildByPath(i + "/obj").iterator();
            ...
            while(true) {
                  ...
                  do {
                     do {
                        while(true) {
                           ...
                           area = (MapleData)var14.next();
                           if (area.getChildByPath("SN_count") != null && area.getChildByPath("speed") != null) {
                              sn_count = MapleDataTool.getIntConvert("SN_count", area, 0);
                              name = MapleDataTool.getString("name", area, "");
                              speed = MapleDataTool.getIntConvert("speed", area, 0);
                              break;
                           }

                           String name;
                           if (area.getChildByPath("SN_count") != null) {
                              name = MapleDataTool.getString("name", area, "");
                              MapleNodes.Environment ev = new MapleNodes.Environment(name, MapleDataTool.getIntConvert("x", area, 0), MapleDataTool.getIntConvert("y", area, 0));
                              nodeInfo.addEnvironment(ev);
                           } else if (area.getChildByPath("tags") != null) {
                              name = MapleDataTool.getString("tags", area, "");
               ...
               for(int x = 0; x < sn_count; ++x) {
                  SN.add(MapleDataTool.getIntConvert("SN" + x, area, 0));
               }

               MapleNodes.MaplePlatform mni = new MapleNodes.MaplePlatform(name, MapleDataTool.getIntConvert("start", area, 2), speed, MapleDataTool.getIntConvert("x1", area, 0), MapleDataTool.getIntConvert("y1", area, 0), MapleDataTool.getIntConvert("x2", area, 0), MapleDataTool.getIntConvert("y2", area, 0), MapleDataTool.getIntConvert("r", area, 0), SN);
      ...
      if (mapData.getChildByPath("area") != null) {
         var4 = mapData.getChildByPath("area").iterator();

         while(var4.hasNext()) {
            node = (MapleData)var4.next();
            int x1 = MapleDataTool.getInt(node.getChildByPath("x1"));
            sn_count = MapleDataTool.getInt(node.getChildByPath("y1"));
            int x2 = MapleDataTool.getInt(node.getChildByPath("x2"));
            speed = MapleDataTool.getInt(node.getChildByPath("y2"));
            Rectangle mapArea = new Rectangle(x1, sn_count, x2 - x1, speed - sn_count);
      ...
      if (mapData.getChildByPath("CaptureTheFlag") != null) {
         mc = mapData.getChildByPath("CaptureTheFlag");
         var14 = mc.iterator();
      ...
      if (mapData.getChildByPath("directionInfo") != null) {
         mc = mapData.getChildByPath("directionInfo");

         MapleNodes.DirectionInfo di;
         for(var14 = mc.iterator(); var14.hasNext(); nodeInfo.addDirection(Integer.parseInt(area.getName()), di)) {
            area = (MapleData)var14.next();
            di = new MapleNodes.DirectionInfo(Integer.parseInt(area.getName()), MapleDataTool.getInt("x", area, 0), MapleDataTool.getInt("y", area, 0), MapleDataTool.getInt("forcedInput", area, 0) > 0);
            mc2 = area.getChildByPath("eventQ");
            if (mc2 != null) {
               Iterator var25 = mc2.iterator();

               while(var25.hasNext()) {
                  MapleData event = (MapleData)var25.next();
      ...
      if (mapData.getChildByPath("monsterCarnival") != null) {
         ...
         if (mc.getChildByPath("mobGenPos") != null) {
            var14 = mc.getChildByPath("mobGenPos").iterator();
         ...
         if (mc.getChildByPath("mob") != null) {
            var14 = mc.getChildByPath("mob").iterator();
         ...
         if (mc.getChildByPath("guardianGenPos") != null) {
            var14 = mc.getChildByPath("guardianGenPos").iterator();
         ...
         if (mc.getChildByPath("skill") != null) {
            var14 = mc.getChildByPath("skill").iterator();
