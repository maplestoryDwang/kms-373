...
public class MapleReactorStats {
   private Point tl;
   private Point br;
   private Map<Byte, MapleReactorStats.StateData> stateInfo = new HashMap();
   ...
   public void addState(byte state, int type, Pair<Integer, Integer> reactItem, byte nextState, int timeOut, byte canTouch) {
      this.stateInfo.put(state, new MapleReactorStats.StateData(type, reactItem, nextState, timeOut, canTouch));
   }

   public byte getNextState(byte state) {
      MapleReactorStats.StateData nextState = (MapleReactorStats.StateData)this.stateInfo.get(state);
