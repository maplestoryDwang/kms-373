...
public class Event_DojoAgent {
   ...
   public static boolean warpStartDojo(MapleCharacter c, boolean party) {
      ...
      if (canenter) {
         if (party && c.getParty() != null) {
            Iterator var13 = c.getParty().getMembers().iterator();

            while(var13.hasNext()) {
               MaplePartyCharacter mem = (MaplePartyCharacter)var13.next();
   ...
   public static void failed(MapleCharacter c) {
      ...
      if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
         Iterator var3 = c.getParty().getMembers().iterator();

         while(var3.hasNext()) {
            MaplePartyCharacter mem = (MaplePartyCharacter)var3.next();
   ...
   public static boolean warpNextMap(MapleCharacter c, boolean fromResting, MapleMap currentmap) {
      try {
         ...
         if (!fromResting && map != null) {
            ...
            if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
               var16 = c.getParty().getMembers().iterator();

               while(var16.hasNext()) {
                  mem = (MaplePartyCharacter)var16.next();
                  ...
                  if (chr != null) {
                     ...
                     point = chr.getIntRecord(150100) + point;
                     chr.getQuestNAdd(MapleQuest.getInstance(150100)).setCustomData(String.valueOf(point));
                     chr.getClient().getSession().writeAndFlush(CWvsContext.Mulung_Pts(point, point));
                  }
               }
            } else {
               basemap = points * 4;
               c.getTrait(MapleTrait.MapleTraitType.will).addExp(points, c);
               c.modifyCSPoints(1, basemap * 4, true);
               x = c.getIntRecord(150100) + basemap;
               c.getQuestNAdd(MapleQuest.getInstance(150100)).setCustomData(String.valueOf(x));
               c.getClient().getSession().writeAndFlush(CWvsContext.Mulung_Pts(basemap, x));
            }
         }
         ...
         if (currentmap.getId() >= 925023800 && currentmap.getId() <= 925023814) {
            mappz = ch.getMapFactory().getMap(925020003);
            if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
               var19 = c.getParty().getMembers().iterator();

               while(var19.hasNext()) {
                  mem = (MaplePartyCharacter)var19.next();
                  chr = currentmap.getCharacterById(mem.getId());
                  if (chr != null) {
                     if (!chr.isAlive()) {
                        chr.addHP(50L);
                     ...
                     chr.changeMap(mappz, mappz.getPortal(1));
                     point = points * 3;
                     c.getTrait(MapleTrait.MapleTraitType.will).addExp(points, c);
               ...
               c.changeMap(mappz, mappz.getPortal(1));
               x = points * 4;
               c.getTrait(MapleTrait.MapleTraitType.will).addExp(points, c);
               int dojo = c.getIntRecord(150100) + x;
               c.getQuestNAdd(MapleQuest.getInstance(150100)).setCustomData(String.valueOf(dojo));
               c.getClient().getSession().writeAndFlush(CWvsContext.Mulung_Pts(x, dojo));
         ...
         if (map != null) {
            ...
            for(x = 0; x < 10; ++x) {
               ...
               if (mapz.getCharactersSize() == 0) {
                  clearMap(mapz, false);
                  if (c.getParty() != null) {
                     Iterator var23 = c.getParty().getMembers().iterator();

                     while(var23.hasNext()) {
                        MaplePartyCharacter mem = (MaplePartyCharacter)var23.next();
                        MapleCharacter chr = currentmap.getCharacterById(mem.getId());
                        if (chr != null) {
                           if (!chr.isAlive()) {
                              chr.addHP(50L);
         ...
         mappz = ch.getMapFactory().getMap(925020001);
         if (c.getParty() != null) {
            var19 = c.getParty().getMembers().iterator();
         ...
         FileoutputUtil.outputFileError("Log_Packet_Except.rtf", var15);
      }
